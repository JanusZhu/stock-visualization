...
export default class StockSearch {
  ...
  init(addCompanyCallback) {
    ...
    // 绑定添加公司按钮点击事件
    document
      .querySelector(".add-company button")
      .addEventListener("click", this.addCompany.bind(this));
  }
  ...
  handleSearchInput() {
    ...
    // 获取查询的拼音
    const queryPinyin = window.pinyinPro
      .pinyin(query, { toneType: "none", type: "array" })
      .join("")
      .toLowerCase();

    // 对每个股票进行评分
    const scoredMatches = this.stockDatabase
      .map((stock) => {
        let score = 0;
        let matchReason = [];
        ...
        // 检查拼音匹配
        const namePinyin = window.pinyinPro
          .pinyin(stock.name, { toneType: "none", type: "array" })
          .join("")
          .toLowerCase();
        ...
        const similarityScore = Math.max(
          ...
          ...stock.alias.map((alias) =>
            this.similarity(query, alias.toLowerCase())
          )
        );
    ...
    if (scoredMatches.length > 0) {
      this.searchResults.innerHTML = scoredMatches
        .map(
          (item, index) => `
                <div class="search-result-item ${
                  index === 0 ? "suggested" : ""
                }" data-code="${item.stock.code}" data-name="${
            item.stock.name
          }">
                    <div>
  ...
  handleSearchResultClick(item) {
    ...
    if (this.addCompanyCallback) {
      this.addCompanyCallback({ code, name });
    }
  }
  ...
  addCompany() {
    ...
    // 尝试匹配输入的股票
    const matchingStocks = this.stockDatabase.filter(
      (stock) =>
        stock.code.toLowerCase() === input.toLowerCase() ||
        stock.name.toLowerCase().includes(input.toLowerCase()) ||
        stock.alias.some((alias) => alias.toLowerCase() === input.toLowerCase())
    );

    // 如果找到匹配的股票
    if (matchingStocks.length > 0) {
      const stockInfo = matchingStocks[0];
      this.searchInput.value = "";

      if (this.addCompanyCallback) {
        this.addCompanyCallback({ code: stockInfo.code, name: stockInfo.name });
      }
    }
    // 如果是有效的股票代码格式但不在数据库中
    else if (/^\d{4}\.[A-Z]{2}$/.test(input)) {
      // 可能是直接输入的股票代码
      this.searchInput.value = "";
      ...
      if (this.addCompanyCallback) {
        ...
        this.addCompanyCallback({ code: input, name: name });
      }
    }
    // 没有找到匹配的股票
    else {
